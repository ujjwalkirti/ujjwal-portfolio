...
const NoteWorthyProjectsCard = ({ project }) => {
  return (
    ...
    >
      ...
      <p className="font-semibold text-gray-900 text-[20px] mb-4 hover:text-blue-600">
        {project.title}
      </p>
      <p className="text-[16px] text-gray-600">{project.desc}</p>
      <div className="break-words mt-[25px] mb-[10px] text-[13px] flex flex-wrap text-gray-900 gap-2">
        {project.tags.map((tag: string, index: number) => (
          <span
            key={index}
            className="flex items-center rounded-full bg-blue-50 px-3 py-1 text-xs font-medium leading-5 text-blue-600 "
          >
            {tag}
          </span>
        ))}
