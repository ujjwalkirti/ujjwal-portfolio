...
function ExperienceCard({ exp }: ExperienceCardProps) {
    ...
    return (
        <li onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="mb-12">
            <Link href={exp.website} target="_blank" rel="noopener noreferrer" aria-label={exp.title} title={exp.title} className="group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50 hover:cursor-pointer">
                ...
                <div className="z-10 sm:col-span-6">
                    ...
                    <List className={`mt-2 text-sm leading-normal ${isHovered ? 'grayish-text' : 'dark-grayish-text'}`}>
                        {exp.description.map((desc, descIndex) => (
                            <ListItem my={2} key={descIndex}>{desc}</ListItem>
                        ))}
                    </List>
