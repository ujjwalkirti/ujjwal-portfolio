    ...
    return (
        <li onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="mb-12">
            <Link href={exp.website} target="_blank" rel="noopener noreferrer" aria-label={exp.title} title={exp.title} className="group relative grid px-3 pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50 hover:cursor-pointer">
                <div className="absolute -inset-x-4 -inset-y-4 z-0 hidden rounded-md transition motion-reduce:transition-none lg:-inset-x-6 lg:block lg:group-hover:bg-gray-50 lg:group-hover:shadow-[inset_0_1px_0_0_rgba(148,163,184,0.1)] lg:group-hover:drop-shadow-lg"></div>
                <header
                ...
                <div className="z-10 sm:col-span-6">
                    <h3 className={`font-medium leading-snug  ${isHovered ? 'text-blue-600 focus-visible:text-blue-600' : 'text-gray-900'}`}>
                    ...
                    <ul className={`mt-2 text-sm list-disc pl-5 leading-normal ${isHovered ? 'text-gray-900' : 'text-gray-600'}`}>
                    ...
                    <ul className="mt-2 flex flex-wrap" aria-label="Technologies used">
                        {exp.technologies.map((tech, techIndex) => (
                            <li key={techIndex} className="mr-1.5 mt-2">
                                <div className="flex items-center rounded-full bg-blue-50 px-3 py-1 text-xs font-medium leading-5 text-blue-600">
                                    {tech}
                                </div>
                            </li>
                        ))}
